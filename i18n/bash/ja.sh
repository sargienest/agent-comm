...
ac_i18n_register "message.command_pending" $'{task_author_role} を読め。特に最重要禁止事項を必ず守れ。\n新しい指示があります。{command_file} を確認し、まず investigation と analyst の調査タスクを配布してください。完了後の result_artifact_path を基に実装タスクへ分解してください。\n\ncommand:\n{command_text}'
...
ac_i18n_register "message.report_research_summary" $'{task_author_role} を読め。特に最重要禁止事項を必ず守れ。\ninvestigation / analyst の完了タスクがあります。以下の結果ファイルを確認して task_author が次の実装タスクを分解してください。\n{summary_lines}'
